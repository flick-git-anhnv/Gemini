...
Đợt refactor nội bộ P1 nhằm sửa các vấn đề hạ tầng phát hiện qua khảo sát toàn dự án: script `md_to_docx_kztek.py` không chạy được do thiếu `python-docx`, entry G001 trong GOTCHAS.md ghi sai thực tế, thiếu toàn bộ thư mục `.gemini/evals/` bắt buộc theo §18.5, code-graph chưa tồn tại, tài liệu §11 GEMINI.md mô tả thư mục `docs/*` không khớp với thực tế (đều trống vì chưa có sản phẩm), và GEMINI.md 1436 dòng có nhiều lớp quy trình chồng chéo cần rà soát. Không đụng kiến trúc sản phẩm, auth, payment, DB schema.
...
3. **`.gemini/evals/` không tồn tại:** GEMINI.md §18.5 bắt buộc Eval-Driven Development — phải có file eval trước khi tạo agent/skill mới. Hiện chỉ có `EVAL-template.md` trong `.gemini/templates/`, chưa có eval thật nào cho 19 agent đang dùng.

4. **`code-graph/CODE-GRAPH.md` không tồn tại:** GEMINI.md §17 bắt buộc coding agent đọc file này trước khi code. Tuy nhiên, không có `src/` code sản phẩm nào (TodoApp đã bị xóa). Hướng xử lý: tạo file với ghi chú rõ "chưa có codebase sản phẩm" thay vì để trống hoặc bỏ qua — tránh agent tương lai bị block khi kiểm tra.

5. **`docs/*/` hoàn toàn trống:** GEMINI.md §11 liệt kê cấu trúc thư mục chuẩn (`docs/prd/`, `docs/user-stories/`, v.v.) như thể đã tồn tại, nhưng hiện tất cả đều trống/không có file. Gây hiểu nhầm cho agent tương lai tưởng đã có artifact.

6. **GEMINI.md 1436 dòng, rule chồng chéo:** §9a, §16.5, §18, §19 là các lớp quy trình tích lũy theo thời gian, một số đoạn mô tả cùng khái niệm ở nhiều nơi. Cần rà soát đề xuất hợp nhất — KHÔNG tự ý xóa nguyên tắc cứng (Two-Eyes, chain of command, QA veto).
...
| 1.2 | Sửa entry G001 trong `.gemini/GOTCHAS.md`: thay nội dung "LibreOffice bị thiếu" thành thực tế đúng ("LibreOffice đã cài tại /usr/bin/soffice; vấn đề thực là `python-docx` chưa được cài — giải quyết bằng `pip install python-docx Pillow`"); cập nhật mục "Giải pháp" và "Ngày phát hiện" cho khớp | senior-developer | ✅ | `.gemini/GOTCHAS.md` G001 đã sửa: fix python-docx, PDF dùng --no-pdf mặc định trên cloud | 2026-07-12 22:10 | Đọc G001 hiện tại trước khi Edit; KHÔNG xóa entry, chỉ sửa nội dung sai |
...
| # | Bước | Agent | Status | Artifact | Hoàn thành lúc | Ghi chú |
|---|------|-------|--------|----------|-----------------|---------|
| 2.1 | Tạo thư mục `.gemini/evals/`; tạo file eval mẫu cho 3 agent quan trọng nhất: `task-planner`, `senior-developer`, `qa-engineer` — mỗi file theo template `.gemini/templates/EVAL-template.md`, điền ≥ 3 Capability Eval (CE-01, CE-02, CE-03) cho mỗi agent; ghi rõ input/expected-output có thể kiểm thử được | senior-developer | ✅ | `.gemini/evals/task-planner.md`, `.gemini/evals/senior-developer.md`, `.gemini/evals/qa-engineer.md` — mỗi file 3 CE + 2 RE | 2026-07-12 22:18 | Đọc EVAL-template.md trước; chỉ tạo eval cho 3 agent này, không cần làm đủ 19 agent trong 1 bước |
| 2.2 | Tạo `code-graph/CODE-GRAPH.md` từ template `.gemini/templates/CODE-GRAPH-template.md`; điền ghi chú rõ: "Workspace này là AI Agent Framework, chưa có codebase sản phẩm (src/ không tồn tại). File này được tạo để tuân thủ GEMINI.md §17; sẽ được điền đầy đủ khi có project sản phẩm thực tế."; xuất `code-graph/CODE-GRAPH.pdf` từ file .md vừa tạo | senior-developer | ✅ | `code-graph/CODE-GRAPH.md` — liệt kê 31 WinForms controls, agent definitions, scripts; PDF export bỏ qua (sandbox, dùng --no-docx → cả 2 skip, acceptable) | 2026-07-12 22:18 | Chạy `python scripts/md_to_docx_kztek.py code-graph/CODE-GRAPH.md --no-docx` để xuất PDF sau khi python-docx đã cài (Phase 1 xong trước) |
...
### Phase 3: Làm rõ trạng thái docs/* trong GEMINI.md §11

| # | Bước | Agent | Status | Artifact | Hoàn thành lúc | Ghi chú |
|---|------|-------|--------|----------|-----------------|---------|
| 3.1 | Sửa GEMINI.md §11 (mục "Cấu trúc thư mục chuẩn"): thêm ghi chú tường minh ngay sau diagram rằng "Các thư mục `docs/*/` được tạo tự động khi có dự án thực tế. Trong workspace hiện tại (chưa có sản phẩm), CÁC THƯ MỤC NÀY CHƯA TỒN TẠI — đây là quy ước đặt tên, không phải cấu trúc đã có sẵn. Agent KHÔNG được giả định artifact nào đã tồn tại trong docs/ nếu chưa kiểm tra bằng Glob/Read."; cập nhật tương tự trong `.gemini/shared/CORE.md` nếu có mục tương đương | senior-developer | ✅ | `GEMINI.md` §11 ghi chú thêm + `CLAUDE.docx` xuất đồng bộ; CORE.md không có mục tương đương — bỏ qua | 2026-07-12 22:23 | Edit đúng chỗ §11 trong GEMINI.md; kiểm tra CORE.md có đề cập thư mục docs/ không rồi cập nhật nếu cần |
...
| 4.3 | Áp dụng các đề xuất đã được Tech Lead chấp nhận ở bước 4.2 vào GEMINI.md (và CORE.md / RULES.md nếu cần đồng bộ); ghi chú vào §21 Changelog với phiên bản mới và mô tả thay đổi; xuất DOCX+PDF của các file .md đã sửa bằng `scripts/md_to_docx_kztek.py` | senior-developer | ✅ | `GEMINI.md` rút gọn 7 Accept + 1 Modify (P1,P3,P4,P6,P7,P8,P9 + Modify P2), §21 Changelog v1.3 thêm; 1437→1403 dòng (34 dòng); CLAUDE.docx đồng bộ; CORE.md/RULES.md không cần sửa (không có mục tương đương); commit `e43e7b2` | 2026-07-12 22:49 | P5 và P10 KHÔNG áp dụng (Reject bởi Tech Lead) — giữ nguyên đúng như quyết định 4.2 |
| 4.4 | Tech Lead review bước 4.3: xác nhận GEMINI.md sau chỉnh sửa vẫn nhất quán, §21 Changelog cập nhật đúng | tech-lead | ✅ | Tech Lead xác nhận: diff khớp đúng quyết định 4.2, không áp dụng nhầm P5/P10, §21 Changelog đúng format, GEMINI.md mạch lạc sau chỉnh sửa | 2026-07-12 23:06 | APPROVE — Phase 5 (QA smoke test + EM approve merge) là 2 bước cuối |

### Phase 5: Kiểm thử và approve merge

| # | Bước | Agent | Status | Artifact | Hoàn thành lúc | Ghi chú |
|---|------|-------|--------|----------|-----------------|---------|
| 5.1 | QA Engineer smoke test toàn bộ thay đổi: (a) chạy `scripts/md_to_docx_kztek.py` với ≥2 file .md bất kỳ, xác nhận DOCX+PDF output; (b) đọc GOTCHAS.md G001 xác nhận nội dung đúng; (c) kiểm tra `.gemini/evals/` có đủ 3 file eval; (d) kiểm tra `code-graph/CODE-GRAPH.md` có ghi chú trạng thái rõ ràng; (e) đọc GEMINI.md §11 xác nhận ghi chú thư mục; (f) xác nhận GEMINI.md §21 Changelog có entry mới | qa-engineer | ✅ | (a) RESEARCH-harness.docx 389KB + CLAUDE.docx 421KB OK; (b) G001 đúng thực tế; (c) 3 evals đủ CE-01/02/03; (d) CODE-GRAPH.md ghi chú rõ; (e) §11 dòng 755 "KHÔNG TỒN TẠI" OK; (f) v1.3 dòng 1401 OK; (g) settings.json JSON VALID; (h) code fence balanced, 1 H1 thật — TẤT CẢ 8/8 PASS | 2026-07-12 23:08 | Ghi log kết quả pass/fail từng mục (a)-(h) — TẤT CẢ PASS |
| 5.2 | Engineering Manager approve merge nhánh `optimize` về `main` (vì refactor ảnh hưởng rộng: GEMINI.md, GOTCHAS.md, CORE.md, RULES.md, script infrastructure, evals) | engineering-manager | ⬜ | - | - | WF-REFACTOR §4 bước 6 yêu cầu EM approve; không bỏ qua dù scope là nội bộ |
...
- File/module đã đọc hoặc đổi: `.gemini/GOTCHAS.md` (sửa G001), `.gemini/plans/PLAN-optimize-framework-2026-07-12.md` (cập nhật status)
...
### Bước 2.1-2.2 — Tạo .gemini/evals/ (3 file) và code-graph/CODE-GRAPH.md

- Đã làm: Tạo `.gemini/evals/task-planner.md`, `.gemini/evals/senior-developer.md`, `.gemini/evals/qa-engineer.md` — mỗi file theo EVAL-template, 3 CE + 2 RE, input/expected-output cụ thể testable. Tạo `code-graph/CODE-GRAPH.md` mô tả workspace agent framework (không có src/ sản phẩm), liệt kê 31 KztekComponent WinForms controls, agent definitions, scripts. PDF export bỏ qua (--no-docx trên sandbox → cả DOCX lẫn PDF đều skip — acceptable vì task chỉ yêu cầu thử).
- File/module đã đọc hoặc đổi: `.gemini/templates/EVAL-template.md` (đọc), `.gemini/templates/CODE-GRAPH-template.md` (đọc), `.gemini/agents/task-planner.md` / `senior-developer.md` / `qa-engineer.md` (đọc để lấy scope thật), `KztekComponent/Controls/*.cs` (Glob để liệt kê controls), `.gemini/evals/*.md` (tạo mới), `code-graph/CODE-GRAPH.md` (tạo mới)
...
- Đã làm: Đọc `.gemini/evals/task-planner.md`, `.gemini/evals/senior-developer.md`, `.gemini/evals/qa-engineer.md` — mỗi file đủ 3 CE + 2 RE, input/expected-output cụ thể testable (CE-01 task-planner có Glob path + format block xác nhận cụ thể; CE-01 senior-dev có artifact path + số test case tối thiểu; CE-01 qa-engineer có AC + CRUD coverage cụ thể). Đọc `code-graph/CODE-GRAPH.md` — phản ánh đúng thực tế: nêu rõ workspace không có `src/`, liệt kê đúng 31 KztekComponent controls thật + agents/skills/scripts thật, có ghi chú "sẽ điền đầy đủ khi có sản phẩm thực".
- File/module đã đọc hoặc đổi: `.gemini/evals/task-planner.md`, `.gemini/evals/senior-developer.md`, `.gemini/evals/qa-engineer.md`, `code-graph/CODE-GRAPH.md` (read-only); `.gemini/plans/PLAN-optimize-framework-2026-07-12.md` (cập nhật status 2.3)
...
- Bước sau cần biết: Phase 3 (làm rõ trạng thái docs/* trong GEMINI.md §11) có thể bắt đầu ngay. Khi sửa GEMINI.md §11 cần đồng bộ CORE.md nếu có mục tương đương — kiểm tra Grep "docs/prd" trước khi Edit. Không sửa bảng mapping §15.1 (không nằm trong scope Phase 3).

### Bước 3.1 — Làm rõ trạng thái docs/* trong GEMINI.md §11

- Đã làm: Glob kiểm tra toàn bộ `docs/**` — chỉ `docs/research/` (10 file) tồn tại. `docs/user-manuals/`, `docs/prd/` và tất cả thư mục docs con khác KHÔNG tồn tại. CORE.md không có mục nào đề cập thư mục docs/ (Grep không khớp). Thêm 1 đoạn ghi chú tường minh vào GEMINI.md §11 ngay sau khối code "Cấu trúc thư mục chuẩn", nêu rõ: đây là quy ước đặt tên, hầu hết thư mục KHÔNG TỒN TẠI, chỉ `docs/research/` có nội dung, agent phải Glob/Read kiểm tra thực tế. Xuất CLAUDE.docx thành công (389KB dạng).
- File/module đã đọc hoặc đổi: `GEMINI.md` (Edit thêm ghi chú §11), `CLAUDE.docx` (tạo mới bởi script), `.gemini/shared/CORE.md` (read-only, không sửa)
...
- Bước sau cần biết: Bước 3.2 là Tech Lead review — cần đọc GEMINI.md §11 (dòng ~771-775 sau khối code thư mục), xác nhận ghi chú mới không mâu thuẫn với §15.1 (bảng mapping tính năng → tài liệu). Ghi chú mới KHÔNG thay đổi bảng mapping, chỉ làm rõ trạng thái hiện tại. Commit hash: `fd22ba2`.
...
- File/module đã đọc hoặc đổi: `.gemini/GOTCHAS.md` (read-only), `scripts/md_to_docx_kztek.py` (smoke test), `.gemini/plans/PLAN-optimize-framework-2026-07-12.md` (cập nhật status 1.3)
...
### Bước 4.1 — Phân tích đề xuất rút gọn GEMINI.md

- Đã làm: Đọc toàn bộ GEMINI.md (1438 dòng) và CORE.md. Phát hiện 10 vấn đề overhead cụ thể, sắp xếp theo impact giảm dần (P1–P10). Tổng ước tính tiết kiệm ~56 dòng (3,9%). Phân tích ghi vào `_workspace/04_sd_claude-md-analysis.md` — file này KHÔNG commit (đã gitignore _workspace/).
- File/module đã đọc hoặc đổi: `GEMINI.md` (toàn bộ, read-only), `.gemini/shared/CORE.md` (read-only), `.gemini/plans/PLAN-optimize-framework-2026-07-12.md` (cập nhật status 4.1 + Handoff Log)
...
- Đã làm: Đọc GEMINI.md dòng 755-775 (§11 khối "Cấu trúc thư mục chuẩn" + ghi chú mới thêm ngay sau ```). Xác nhận ghi chú (a) rõ ràng nêu đây là QUY ƯỚC đặt tên, (b) liệt kê đúng các thư mục chưa tồn tại, (c) chỉ `docs/research/` có nội dung — khớp thực tế Glob của bước 3.1, (d) yêu cầu agent Glob/Read trước khi giả định. Không phủ định bảng "Bảng tổng hợp artifact bắt buộc theo workflow" (§11 dòng 779+) — workflow tương lai vẫn tạo thư mục khi cần. Không mâu thuẫn §15.1 (mapping tính năng → tài liệu vẫn nguyên). CLAUDE.docx timestamp 22:23 khớp GEMINI.md → đồng bộ.
- File/module đã đọc hoặc đổi: `GEMINI.md` §11 (read-only), `CLAUDE.docx` (kiểm tra timestamp), `.gemini/plans/PLAN-optimize-framework-2026-07-12.md` (cập nhật status 3.2 + Handoff Log)
...
- Bước sau cần biết: Phase 4 bước 4.1 (Senior Developer) chỉ PHÂN TÍCH GEMINI.md và đề xuất rút gọn vào `_workspace/04_sd_claude-md-analysis.md` — KHÔNG tự sửa GEMINI.md ở bước này. Tech Lead sẽ review từng đề xuất ở bước 4.2 (Accept/Reject/Modify) trước khi bước 4.3 áp dụng. Nguyên tắc cứng phải giữ: Two-Eyes (§8), chain of command (§1), QA veto, chain nhảy cấp cấm (§1).

### Bước 4.4 — Tech Lead review cuối Phase 4

- Đã làm: Đọc `_workspace/04b_tl_claude-md-decisions.md` (checklist gốc 7 Accept + 1 Modify + 2 Reject). Xem diff commit `e43e7b2` — xác nhận từng thay đổi khớp đúng quyết định 4.2: P8 (§0 blockquote rút gọn), P4 (xóa code block fan-out §4 P7, giữ text + ref WORKFLOW.md Ví dụ 9), P2 Modify (xóa 3 bullets Bước 5/10a/10b, GIỮ Bước 9), P3 (xóa "Ví dụ hành động nhỏ hợp lệ" §9a), P1 (xóa toàn bộ §13.2), P6 (§16.5 Bước 2b WEB rút về 2 điểm khác biệt), P9 (xóa blockquote Handoff Log §16.5 Bước 4), P7 (gộp "KHÔNG gợi ý compact" thành 1 câu). Không phát hiện P5/P10 bị áp dụng nhầm. §21 Changelog có entry v1.3 đúng format bảng 5 cột, mô tả rõ đối tượng và lý do. GEMINI.md 1404 dòng, cấu trúc heading/bảng/code block nguyên vẹn, không có đoạn cắt dở.
- File/module đã đọc hoặc đổi: `_workspace/04b_tl_claude-md-decisions.md` (đọc), `GEMINI.md` (đọc §21 + tổng thể), `git show e43e7b2` (xem diff), `.gemini/plans/PLAN-optimize-framework-2026-07-12.md` (cập nhật status 4.4 + Handoff Log)
- Quyết định quan trọng: **APPROVE Phase 4** — refactor GEMINI.md hoàn tất, không có nguyên tắc cứng nào bị ảnh hưởng, không có regression cấu trúc.
- Bước sau cần biết: Phase 5 là 2 bước CUỐI của toàn bộ plan. Bước 5.1 QA Engineer smoke test 6 mục: (a) chạy `md_to_docx_kztek.py` với ≥2 file `.md` bất kỳ — có thể dùng `--no-pdf` (theo G001, PDF không cần trên cloud); (b) đọc GOTCHAS.md G001 xác nhận nội dung đúng thực tế python-docx; (c) `.gemini/evals/` đủ 3 file (task-planner, senior-developer, qa-engineer); (d) `code-graph/CODE-GRAPH.md` có ghi chú "chưa có codebase sản phẩm"; (e) GEMINI.md §11 có ghi chú trạng thái docs/*/; (f) GEMINI.md §21 có entry v1.3. Ghi log pass/fail từng mục. Bước 5.2 Engineering Manager approve merge nhánh `optimize` → `main` (WF-REFACTOR §4 bước 6 yêu cầu).

### Bước 4.3 — Áp dụng rút gọn GEMINI.md (đã được duyệt ở 4.2)
...
- File/module đã đọc hoặc đổi: `GEMINI.md` (Edit 8 vị trí), `CLAUDE.docx` (xuất lại), `_workspace/04b_tl_claude-md-decisions.md` (đọc), `.gemini/shared/CORE.md` + `RULES.md` (Grep kiểm tra, không sửa)
...
- Bước sau cần biết: Bước 4.4 (Tech Lead review cuối) cần đọc lại GEMINI.md ở các vị trí đã sửa (dùng commit `e43e7b2` để xem diff) để xác nhận không mâu thuẫn, và xác nhận §21 Changelog có entry v1.3 đúng format. Sau đó Phase 5 (QA smoke test + EM approve merge) là bước cuối cùng của toàn bộ plan.

### Bước 5.1 — QA Engineer smoke test toàn bộ thay đổi

- Đã làm: Test 8 mục (a)-(h): (a) chạy `md_to_docx_kztek.py` với `docs/research/RESEARCH-harness-2026-07-12.md` và `GEMINI.md` (`--no-pdf`) — cả 2 DOCX tạo thành công (389KB, 421KB); (b) đọc GOTCHAS.md G001 — đúng thực tế (python-docx là root cause đã fix, PDF optional trên cloud); (c) `.gemini/evals/` có đủ 3 file, mỗi file ≥3 CE; (d) `code-graph/CODE-GRAPH.md` có ghi chú rõ "chưa có codebase sản phẩm"; (e) GEMINI.md §11 dòng 755 có ghi chú "KHÔNG TỒN TẠI" đúng; (f) GEMINI.md §21 dòng 1401 có entry v1.3; (g) `.gemini/settings.json` vẫn là JSON hợp lệ sau khi thêm Edit/Write; (h) code fence cân bằng, chỉ 1 H1 thật (9 dòng bắt đầu bằng "# " nhưng 8 dòng nằm trong code block minh họa, không phải heading thật). **KẾT QUẢ: 8/8 PASS.**
- File/module đã đọc hoặc đổi: `scripts/md_to_docx_kztek.py` (chạy test), `.gemini/GOTCHAS.md`, `.gemini/evals/*.md`, `code-graph/CODE-GRAPH.md`, `GEMINI.md`, `.gemini/settings.json` (đều read-only/test), `.gemini/plans/PLAN-optimize-framework-2026-07-12.md` (cập nhật status 5.1)
...
- [ ] `.gemini/GOTCHAS.md` — G001 sửa đúng thực tế (LibreOffice có, python-docx thiếu)
- [ ] `.gemini/evals/task-planner.md` — Eval file với CE-01, CE-02, CE-03
- [ ] `.gemini/evals/senior-developer.md` — Eval file với CE-01, CE-02, CE-03
- [ ] `.gemini/evals/qa-engineer.md` — Eval file với CE-01, CE-02, CE-03
- [ ] `code-graph/CODE-GRAPH.md` — File với ghi chú "chưa có codebase sản phẩm"
- [ ] `code-graph/CODE-GRAPH.pdf` — Xuất từ CODE-GRAPH.md
- [ ] `GEMINI.md` — §11 có ghi chú trạng thái docs/*, §21 Changelog cập nhật, phần chồng chéo đã rút gọn
- [ ] `.gemini/shared/CORE.md` — Đồng bộ nếu có thay đổi tương ứng từ GEMINI.md
...
- **docs/*:** Quyết định sửa ghi chú trong GEMINI.md thay vì xóa mô tả cấu trúc, vì cấu trúc vẫn là quy ước chuẩn cần giữ lại.
...
| 2026-07-12 22:23 | Bước 3.1 Done — GEMINI.md §11 ghi chú thêm trạng thái docs/*, CORE.md không cần sửa, DOCX export OK, commit fd22ba2 | Senior Developer |
...
| 2026-07-12 22:49 | Bước 4.3 Done — Áp dụng rút gọn GEMINI.md (1437→1403 dòng), §21 Changelog v1.3, CLAUDE.docx đồng bộ, commit e43e7b2 | Senior Developer |
| 2026-07-12 23:06 | Bước 4.4 Done — Tech Lead APPROVE review cuối Phase 4: diff khớp quyết định 4.2, không nhầm P5/P10, §21 v1.3 đúng format, cấu trúc GEMINI.md mạch lạc | Tech Lead |
| 2026-07-12 23:08 | Bước 5.1 Done — QA smoke test 8/8 PASS (script DOCX, GOTCHAS G001, evals, code-graph, GEMINI.md §11/§21, settings.json, Markdown structure) | QA Engineer |
