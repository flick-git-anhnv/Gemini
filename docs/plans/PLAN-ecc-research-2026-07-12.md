...
- Đã làm: Kiểm tra git branch (claude/ecc-research-l0sizv), glob docs/research/ (trống), glob .gemini/plans/ (chỉ có plan mới tạo). Xác nhận task mới, không có artifact trước đó, không có drift.
- File/module đã đọc: `.gemini/plans/PLAN-ecc-research-2026-07-12.md`
...
- Đã làm: User chọn E1-E7, bỏ E8. Áp dụng 7 thay đổi: E1 (hook config-protection.js + settings.json), E2 (GOTCHAS.md + GEMINI.md §KHỞI ĐỘNG), E3 (CORE.md §6b DAILY/LIBRARY table), E4 (verify-pr.md + GEMINI.md WF-BUGFIX/WF-FEATURE), E5 (EVAL-template.md + GEMINI.md §18.5 EDD), E6 (GEMINI.md §9a introspection debugging), E7 (GEMINI.md §16.5 compact gợi ý). Đã cập nhật RESEARCH-ecc-2026-07-12.md trạng thái áp dụng + xuất DOCX. Thêm §21 changelog.
- File/module đã đọc hoặc đổi: `.gemini/hooks/config-protection.js` (tạo), `.gemini/settings.json` (sửa), `.gemini/GOTCHAS.md` (tạo), `GEMINI.md` (sửa nhiều section), `.gemini/shared/CORE.md` (thêm §6b), `.gemini/commands/verify-pr.md` (tạo), `.gemini/templates/EVAL-template.md` (tạo), `docs/research/RESEARCH-ecc-2026-07-12.md` (cập nhật status + trạng thái áp dụng)
...
- [x] `.gemini/hooks/config-protection.js` — E1: Hook bảo vệ config (Node.js, 5/5 test pass)
- [x] `.gemini/settings.json` — E1: Đăng ký PreToolUse hook
- [x] `.gemini/GOTCHAS.md` — E2: Ghi lỗi ngầm (1 entry G001)
- [x] `.gemini/shared/CORE.md` §6b — E3: Bảng DAILY/LIBRARY classification
- [x] `.gemini/commands/verify-pr.md` — E4: Pre-PR verification checklist skill
- [x] `.gemini/templates/EVAL-template.md` — E5: Eval-Driven Development template
- [x] `GEMINI.md` §KHỞI ĐỘNG §9a §16.5 §18.5 §WF-BUGFIX §WF-FEATURE §21 — E2/E4/E5/E6/E7 + changelog
