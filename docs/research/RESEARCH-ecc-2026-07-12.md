---
title: "Nghiên cứu repo GitHub: affaan-m/ecc (Everything Gemini Agent)"
...
# Nghiên cứu repo: affaan-m/ecc — Everything Gemini Agent (ECC)
...
**ECC (Everything Gemini Agent)** là một *harness-native operator system* — hệ điều hành lớp điều phối agent — được thiết kế để cài vào các AI coding harness (Gemini Agent, Codex, OpenCode, Cursor, Gemini, Zed, GitHub Copilot). Đây không phải là framework lập trình thông thường; ECC là một hệ thống chính sách, kỹ năng (skills), quy tắc (rules), hook tự động, và agent chuyên biệt hoạt động phía trên các LLM harness.
...
**Đối tượng sử dụng:** Engineering team và individual developer đang dùng Gemini Agent hoặc các AI coding agent tương đương; muốn áp dụng best practice có hệ thống (TDD, security, code review) mà không phải viết tay toàn bộ prompt/config.
...
├── .gemini/                    # Gemini Agent-specific config
...
```
User Input → Gemini Agent / Codex / Cursor
               ↓
...
|---|---------|-----------|-------------|
| E1 | Hook bảo vệ config | ✅ Đã áp dụng | `.gemini/hooks/config-protection.js`, `.gemini/settings.json` |
| E2 | PLUGIN_SCHEMA_NOTES pattern (GOTCHAS.md) | ✅ Đã áp dụng | `.gemini/GOTCHAS.md`, `GEMINI.md` §KHỞI ĐỘNG |
| E3 | DAILY vs LIBRARY skill classification | ✅ Đã áp dụng | `.gemini/shared/CORE.md` §6b |
| E4 | Verification Loop trước PR | ✅ Đã áp dụng | `.gemini/commands/verify-pr.md`, `GEMINI.md` §WF-FEATURE Bước 10, §WF-BUGFIX Bước 3 |
| E5 | Eval-Driven Development | ✅ Đã áp dụng | `.gemini/templates/EVAL-template.md`, `GEMINI.md` §18.5 |
| E6 | Agent Introspection Debugging | ✅ Đã áp dụng | `GEMINI.md` §9a |
| E7 | Strategic Compact gợi ý | ✅ Đã áp dụng | `GEMINI.md` §16.5 |
