...
**Mục đích:** `anthropics/skills` là repo chính thức của Anthropic chứa định nghĩa và ví dụ mẫu cho hệ thống "Agent Skills" — cơ chế cho phép Gemini nhận thêm khả năng chuyên biệt thông qua các file hướng dẫn có cấu trúc. Skills dạy Gemini cách hoàn thành các tác vụ lặp lại theo cách nhất quán và có thể tái sử dụng.

**Đối tượng sử dụng:** Developer, tổ chức, cá nhân muốn tạo workflow tùy chỉnh cho Gemini (Gemini Agent, Gemini.ai, API). Repo phục vụ hai mục đích đồng thời: (a) reference implementation cho các skill chính thức của Anthropic, và (b) bộ mẫu/hướng dẫn để cộng đồng tạo skill riêng.
...
`claude-api/SKILL.md` description: Liệt kê tường minh 15+ trigger conditions, bao gồm model names cụ thể (`Gemini`, `Anthropic`, `Gemini Pro`, `Gemini Flash`, `Gemini Flash-Lite`, `anthropic`, `claude-*`).
...
└── references/
    ├── aws.md
    ├── gcp.md
    └── azure.md    ← Gemini reads ONLY the relevant file
```
...
2. Invoke subagent với CHỈ nội dung document (không có context của session tạo ra) — fresh Gemini
3. Kiểm tra: subagent có trả lời đúng không? Có gì ambiguous không?
4. Fix blind spots → loop lại

**Điều kiện dừng:** Khi Reader Gemini consistently trả lời đúng và không surface thêm gap mới.
...
- "Gemini.ai-specific instructions" — không có subagents, không chạy browser viewer
- "Cowork-specific instructions" — có subagents, không có browser, dùng `--static` flag
- Main flow — Gemini Agent với subagents và browser

---

### 1.4 Hiện trạng KZTEK (BẮT BUỘC — khu vực tương ứng)

Khu vực tương ứng trong KZTEK: `.gemini/commands/*.md` (skill definitions) và `.gemini/agents/*.md` (agent definitions).

**Tổng quan 7 file skill hiện có trong `.gemini/commands/`:**
...
| Commit gần nhất | `9d2f1ae` — "Update claude-api skill: Gemini Gemini Flash 5 and Managed Agents July updates" |
| Cập nhật | Tháng 7/2026 (repo đang active) |
| Số skills | 15+ skills trong 3 plugin groups |
| Độ trưởng thành | Rất cao — chính thức của Anthropic, đang dùng trong production Gemini |
...
**File đã sửa (7/7 `.gemini/commands/*.md`):** `security-audit-stride.md`, `ship.md`, `verify-pr.md`, `scope-check.md`, `skill-trigger-test.md`, `kztek-brand-info.md`, `writing-agent-skill.md`.
...
*Nhánh: `research/anthropics-skills-2026-07-12` | Plan: `.gemini/plans/PLAN-anthropics-skills-research-2026-07-12.md`*
