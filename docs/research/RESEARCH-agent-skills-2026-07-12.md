...
├── hooks/
├── .gemini/commands/, .gemini/commands/, commands/
├── plugin.json
└── docs/
```
...
Hỗ trợ 70+ agent CLI (Gemini Agent, Cursor, Copilot, Cline, Gemini, ...) qua CLI hoặc marketplace:
...
- Cấu trúc "vòng đời Define → Plan → Build → Verify → Review → Ship" tương đồng ở mức khái niệm với chain-of-command workflow hiện có trong `GEMINI.md` (WF-FEATURE, WF-BUGFIX, ...), nhưng agent-skills tổ chức theo **skill/lifecycle stage** thay vì theo **vai trò tổ chức** (PM/BA/Tech Lead/...).
...
|---|---|---|---|
| 1 | Anti-rationalization table trong security-audit-stride | ✅ Đã áp dụng | `.gemini/commands/security-audit-stride.md` |
| 2 | Red Flags trong tech-lead / senior-dev / qa-engineer | ✅ Đã áp dụng | `.gemini/agents/tech-lead.md`, `.gemini/agents/senior-developer.md`, `.gemini/agents/qa-engineer.md` |
| 3 | Verification checklist cuối skill/command files | ✅ Đã áp dụng | `.gemini/commands/security-audit-stride.md`, `.gemini/commands/scope-check.md`, `.gemini/agents/code-migrator.md` |
| 4 | "When NOT to use" trong skill/agent definitions | ✅ Đã áp dụng | `.gemini/commands/security-audit-stride.md`, `.gemini/agents/code-migrator.md` (`scope-check.md` đã có sẵn "KHÔNG dùng khi") |
| 5 | Tách reference checklists ra `references/` | ✅ Đã áp dụng | `.gemini/references/security-audit-checklist.md` (mới), link từ `security-audit-stride.md` |
| 6 | "ASSUMPTIONS I'M MAKING" format block | ✅ Đã áp dụng | `.gemini/agents/code-migrator.md` (G1), `.gemini/agents/tech-lead.md` (trước TDD) |
| 7 | Severity label protocol trong code review output | ✅ Đã áp dụng | `.gemini/agents/tech-lead.md` |
| 8 | Slash command `/ship` fan-out | ✅ Đã áp dụng | `.gemini/commands/ship.md` (mới), route ghi chú trong `GEMINI.md` §4 |
| 9 | Ghi chú trade-off dispatcher layer | ✅ Đã áp dụng | `GEMINI.md` §0, `.gemini/shared/CORE.md` §1 |
| 10 | `references/definition-of-done.md` standing checklist | ✅ Đã áp dụng | `.gemini/references/definition-of-done.md` (mới) |
